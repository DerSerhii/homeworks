...
```
```sql
select concat('We have ', count(*), case gender when 'm' then ' boys!' when 'f' then ' girls!' end) as "Gender information;" from people group by gender;

OR
select concat('We have ', count(id), ' boys!') as "Genger information:" from people where gender = 'm'
       union
       select concat('We have ', count(id), ' girls!') as "Genger information:" from people where gender = 'f';
```
```
 Genger information:
---------------------
